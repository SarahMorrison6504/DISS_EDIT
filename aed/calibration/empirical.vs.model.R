...
compare_long <- compare_cyno %>%
  pivot_longer(cols = c("Model", "Observed"),
               names_to = "Source",
               values_to = "Cyno")

# Plot
ggplot(compare_long, aes(x = Date, y = Cyno, colour = Source)) +
  ...
  theme(legend.position = "top")
#rmse
rmse <- sqrt(mean((compare_cyno$Model - compare_cyno$Observed)^2, 
                  na.rm = TRUE))
# print rmse
rmse
