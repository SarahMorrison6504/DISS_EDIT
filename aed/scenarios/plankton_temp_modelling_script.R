...
# Calculate mean temperature across depth
temp_mean <- colMeans(temp, na.rm = TRUE)

# Convert NetCDF time (hours since 2016-01-01) to datetime
...
# Plot
ggplot(df, aes(x = date, y = temp_mean)) +
  geom_line(linewidth = 1, colour = "steelblue") +
  labs(
    x = "Date",
    y = "Mean Water Column Temperature (°C)",
    title = "Whole-Lake Mean Temperature"
  ) +
  theme_classic()
