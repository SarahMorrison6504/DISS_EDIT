...
solar_max <- 250   # max solar
solar_range <- solar_max - solar_min
bga_min <- get_var("output_solar0.nc", var_name = "PHY_bga", reference = "surface")
bga_max <- get_var("output_solarMAX.nc", var_name = "PHY_bga", reference = "surface")

calc_deltabga <- function(depth_col) {
  
  df_min <- bga_min[, c("DateTime", depth_col)]
  df_max <- bga_max[, c("DateTime", depth_col)]
  
  colnames(df_min)[2] <- "bga_min"
  colnames(df_max)[2] <- "bga_max"
  
  df <- merge(df_min, df_max, by = "DateTime")
  
  df$deltabga <- df$bga_max - df$bga_min
  
  # Sensitivity coefficient
  df$sensitivity <- df$deltabga / solar_range
  
  return(df)
}

# apply to each depth
surface <- calc_deltabga("PHY_bga_0")
middle  <- calc_deltabga("PHY_bga_2.63435122780277")
bottom  <- calc_deltabga("PHY_bga_4.74183221004499")

delta_all <- data.frame(
  DateTime = surface$DateTime,
  surface = surface$deltabga,
  middle  = middle$deltabga,
  bottom  = bottom$deltabga
)
(delta_bga_surface <- mean((surface$deltabga), na.rm = TRUE))
(delta_bga_middle  <- mean((middle$deltabga), na.rm = TRUE))
(delta_bga_bottom <- mean((bottom$deltabga), na.rm = TRUE))
...
library(patchwork)
# Example with fixed y-axis
y_range <- range(c(surface$deltabga, middle$deltabga, bottom$deltabga), na.rm = TRUE)

bga_solar_surface <- ggplot(surface, aes(x = DateTime, y = deltabga)) +
  geom_line(color = 'deeppink1', size = 1) +
  ylim(y_range) +
  theme_classic() +
  theme(axis.text.x = element_blank(), axis.title.x = element_blank()) +
...
bga_solar_middle <- ggplot(middle, aes(x = DateTime, y = deltabga)) +
  geom_line(color = "maroon", size = 1) +
  ylim(y_range) +
  theme_classic() +
  theme(axis.text.x = element_blank(), axis.title.x = element_blank()) +
  labs(title = "Middle", x = NULL, y = "ΔBGA (mmol/m³)")

bga_solar_bottom <- ggplot(bottom, aes(x = DateTime, y = deltabga)) +
  geom_line(color = "darkred", size = 1) +
  ylim(y_range) +
  theme_classic() +
  labs(title = "Bottom", x = "Date", y = NULL)

(bga_solar_surface / bga_solar_middle / bga_solar_bottom) +
  plot_annotation(
    title = "Blue Green Algae Sensitivity to Shortwave Radiation Extremes")
...
(mean_sensitivity_surface <- mean(abs(surface_wind$sensitivity), na.rm = TRUE))
(mean_sensitivity_middle  <- mean(abs(middle_wind$sensitivity), na.rm = TRUE))
(mean_sensitivity_bottom  <- mean(abs(bottom_wind$sensitivity), na.rm = TRUE))
...
(mean_sensitivity_surface <- mean(abs(surface_nit$sensitivity), na.rm = TRUE))
(mean_sensitivity_middle  <- mean(abs(middle_nit$sensitivity), na.rm = TRUE))
(mean_sensitivity_bottom  <- mean(abs(bottom_nit$sensitivity), na.rm = TRUE))
...
(mean_sensitivity_surface <- mean(abs(surface_phs$sensitivity), na.rm = TRUE))
(mean_sensitivity_middle  <- mean(abs(middle_phs$sensitivity), na.rm = TRUE))
(mean_sensitivity_bottom  <- mean(abs(bottom_phs$sensitivity), na.rm = TRUE))
...
(mean_sensitivity_surface <- mean(abs(surface_do$sensitivity), na.rm = TRUE))
(mean_sensitivity_middle  <- mean(abs(middle_do$sensitivity), na.rm = TRUE))
(mean_sensitivity_bottom  <- mean(abs(bottom_do$sensitivity), na.rm = TRUE))
...
(mean_sensitivity_surface <- mean(abs(surface_cyno$sensitivity), na.rm = TRUE))
(mean_sensitivity_middle  <- mean(abs(middle_cyno$sensitivity), na.rm = TRUE))
(mean_sensitivity_bottom  <- mean(abs(bottom_cyno$sensitivity), na.rm = TRUE))
...
(mean_sensitivity_surface <- mean(abs(surface_diat$sensitivity), na.rm = TRUE))
(mean_sensitivity_middle  <- mean(abs(middle_diat$sensitivity), na.rm = TRUE))
(mean_sensitivity_bottom  <- mean(abs(bottom_diat$sensitivity), na.rm = TRUE))
...
wind_max_val <- 15    # max wind
wind_range   <- wind_max_val - wind_min_val
bga_min <- get_var("output_windMIN.nc", var_name = "PHY_bga", reference = "surface")
bga_max <- get_var("output_windMAX.nc", var_name = "PHY_bga", reference = "surface")

calc_delta_bga <- function(depth_col_min, depth_col_max) {
  
  df_min <- bga_min[, c("DateTime", depth_col_min)]
  df_max <- bga_max[, c("DateTime", depth_col_max)]
  ...
  # Sensitivity coefficient
  df$sensitivity <- df$deltabga / wind_range
...
colnames(bga_min)
colnames(bga_max)
surface_bga <- calc_delta_bga("PHY_bga_0", "PHY_bga_0")

middle_bga  <- calc_delta_bga("PHY_bga_2.63472105263158", 
                                "PHY_bga_2.63296309693378")

bottom_bga  <- calc_delta_bga("PHY_bga_4.74249789473684", 
                                "PHY_bga_4.7393335744808")

delta_all <- data.frame(
  DateTime = surface_bga$DateTime,
  surface_bga = surface_bga$deltabga,
  middle_bga  = middle_bga$deltabga,
  bottom_bga  = bottom_bga$deltabga
)

(delta_bga_surface <- mean((surface_bga$deltabga), na.rm = TRUE))
(delta_bga_middle  <- mean((middle_bga$deltabga), na.rm = TRUE))
(delta_bga_bottom <- mean((bottom_bga$deltabga), na.rm = TRUE))

(mean_sensitivity_surface <- mean(abs(surface_bga$sensitivity), na.rm = TRUE))
(mean_sensitivity_middle  <- mean(abs(middle_bga$sensitivity), na.rm = TRUE))
(mean_sensitivity_bottom  <- mean(abs(bottom_bga$sensitivity), na.rm = TRUE))

# plot
library(ggplot2)
library(patchwork)
# Example with fixed y-axis
y_range <- range(c(surface_bga$deltabga, middle_bga$deltabga, bottom_bga$deltabga), na.rm = TRUE)

p_surface <- ggplot(surface_bga, aes(x = DateTime, y = deltabga)) +
  geom_line(color = '#339999', size = 1) +
  ylim(y_range) +
  theme_classic() +
  theme(axis.text.x = element_blank(), axis.title.x = element_blank()) +
...
p_middle <- ggplot(middle_bga, aes(x = DateTime, y = deltabga)) +
  geom_line(color = '#336666', size = 1) +
  ylim(y_range) +
  theme_classic() +
  theme(axis.text.x = element_blank(), axis.title.x = element_blank()) +
  labs(title = "Middle", x = NULL, y = "ΔBlue Green Algae (mmol/m³)")

p_bottom <- ggplot(bottom_bga, aes(x = DateTime, y = deltabga)) +
  geom_line(color = "darkblue", size = 1) +
  ylim(y_range) +
  theme_classic() +
  labs(title = "Bottom", x = "Date", y = NULL)
...
(mean_sensitivity_surface <- mean(abs(surface_green$sensitivity), na.rm = TRUE))
(mean_sensitivity_middle  <- mean(abs(middle_green$sensitivity), na.rm = TRUE))
(mean_sensitivity_bottom  <- mean(abs(bottom_green$sensitivity), na.rm = TRUE))
